...
// Include tha type transformations from and to nat.
#include <int6x_nat.h>
...
value in8(struct thread_info *tinfo, value coq_reg_id)
{
  ...
  uint8_t reg_id = (uint8_t)int64_from_nat(coq_reg_id);
  trace("[certcspkr]: in8: Reading from 0x%x\n", reg_id);
  // Call in8 and transform the return val to value type, i.e. nat
  return int64_to_nat(tinfo, (uint64_t)do_in8(reg_id));
}
...
value in8_p(struct thread_info *tinfo, value coq_reg_id)
{
  ...
  // Call in8 and transform the return val to value type, i.e. nat
  return int64_to_nat(tinfo, (uint64_t)do_in8_p(reg_id));
}
...
value out8(struct thread_info *tinfo, value coq_reg_val, value coq_reg_id)
{
  ...
  uint8_t reg_val = (uint8_t)int64_from_nat(coq_reg_val);
  uint8_t reg_id  = (uint8_t)int64_from_nat(coq_reg_id);
